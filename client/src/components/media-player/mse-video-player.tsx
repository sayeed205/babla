...
export const MSEVideoPlayer: React.FC<MSEVideoPlayerProps> = ({
    ...
    const estimateMemoryUsage = useCallback((): number => {
        try {
            if (!state.sourceBuffer || !state.mediaSource || state.mediaSource.readyState !== 'open') return 0
    ...
    const performMemoryCleanup = useCallback(() => {
        try {
            if (!state.sourceBuffer || !videoRef.current || !state.mediaSource || 
    ...
    const handleError = useCallback(
        (error: string | StreamingError, canRetry: boolean = false) => {
            ...
            const errorContext = {
                ...
                bufferHealth: state.sourceBuffer ? `${state.sourceBuffer.buffered.length} ranges` : 'no buffer',
    ...
    const loadNextChunks = useCallback(
        (currentTime: number) => {
            ...
            if (currentMemoryUsage > MAX_BUFFER_SIZE) {
                console.warn(`Memory usage too high (${(currentMemoryUsage / 1024 / 1024).toFixed(1)}MB), skipping chunk loading`)
                return
            }
    ...
    useEffect(() => {
        if (src && !authLoading && isAuthenticated && token?.token &&
