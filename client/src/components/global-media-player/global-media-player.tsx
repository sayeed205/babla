...
export function GlobalMediaPlayer() {
    ...
    // Enhanced MediaSource cleanup function to prevent memory leaks and ensure proper state management
    const cleanupMediaSource = useCallback(() => {
        ...
        if (currentMediaSourceRef.current) {
            try {
                ...
                console.log('MediaSource ended and event listeners removed')
            } catch (error) {
        ...
        // Clean up audio elements (for LosslessAudioPlayer)
        ...
        console.log('Complete media cleanup performed')
    }, [])

    // Ensure only one MediaSource is active per video element
    const ensureSingleMediaSource = useCallback((newMediaId: string) => {
        if (activeMediaSourceId && activeMediaSourceId !== newMediaId) {
            console.log(`Cleaning up previous MediaSource (${activeMediaSourceId}) for new media (${newMediaId})`)
            cleanupMediaSource()
        }
    ...
    const handleMediaTransition = useCallback(async (newMedia: typeof currentMedia) => {
        if (!newMedia) return
        ...
        if (isSameMedia) {
            console.log('Same media detected, skipping transition')
            return
        }
        ...
        try {
            ...
            if (previousMedia && previousMedia.id !== newMedia.id) {
                console.log(`Switching from ${previousMedia.title} to ${newMedia.title}`)
                cleanupMediaSource()
            ...
            console.log(`Transitioning to new media: ${newMedia.title} (${detectMediaType(newMedia)})`)
        } catch (error) {
    ...
    const cleanupOldBuffers = useCallback(() => {
        ...
        if (buffered.length > 0) {
            console.log(`Buffer ranges: ${buffered.length}, Current time: ${currentTime.toFixed(2)}s`)
        ...
        // This function provides monitoring and logging for the global player
    ...
    const handlePlayerLoadedMetadata = useCallback(() => {
        ...
        console.log('Media loaded successfully in global player')
    }, [])
    ...
    const validatePlayerState = useCallback(() => {
        ...
        if (elementPlaying !== storePlayingState) {
            console.warn('Player state inconsistency detected, syncing...')
            setIsPlaying(elementPlaying)
        }

        // Check for stuck loading states
        if (isPlayerLoading && mediaElement.readyState >= 3) { // HAVE_FUTURE_DATA
            console.warn('Stuck loading state detected, clearing...')
            setIsPlayerLoading(false)
        }
    }, [currentMedia, isPlayerVisible, getCurrentMediaElement, isPlaying, isPlayerLoading, setIsPlaying])
    ...
    // Periodic buffer cleanup and health monitoring
    useEffect(() => {
        if (!isPlayerVisible || !currentMedia) return
    ...
    }, [isPlayerVisible, currentMedia, cleanupOldBuffers])
    ...
    useEffect(() => {
        return () => {
            ...
            console.log('Global media player unmounted and cleaned up')
        }
    }, [cleanupMediaSource])
